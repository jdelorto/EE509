...
```{r task2}
negLogLike <- function(x,y,param){
  -sum(dbinom(y,1,pnorm(x,param[1],param[2]),log=TRUE))
}

negLogLike(dia,cones,param)

fit <- optim(param,negLogLike,dia,cones,param)
...
```{r}
param  <- c(a0,fit$par[1],fit$par[2])        ## initial guess (update this based on the first two models!)
...
lines(xseq,a0*xseq^2,col="forestgreen",lwd=2)
lines(xseq,a0*xseq^2*pnorm(xseq,b0,b1),col="blue",lwd=2)
legend("topleft",legend=c("fecundity given maturity: g(x)","expected # of cones"),col=c("forestgreen","blue"),lwd=2)

plot(dia,cones,main="Maturation")
lines(xseq,pnorm(xseq,b0,b1),col=2,lwd=2)
```
...
```{r task4}
dia.co2 <- b$diam[tmt=="CO2"]
ncone.co2 <- b$c00[tmt=="CO2"]
...
n[3] <- length(ncone.co2)
out.elev

parameters <- matrix(c(a0,b0,b1,lnL,n),ncol=5)
colnames(parameters) = c("a0","b0","b1","lnL","n")
rownames(parameters) = c("all","ambient","co2")
...
```{r task5}
plot(dia,ncone,main="Fecundity")
lines(xseq,a0[1]*xseq^2,col=1,lwd=2)
lines(xseq,a0[2]*xseq^2,col=2,lwd=2)
lines(xseq,a0[3]*xseq^2,col=3,lwd=2)
legend("topleft",legend=c("Full model","Ambient","CO2"),col=1:3,lwd=2)

plot(dia,cones,main="Maturity")
lines(xseq,pnorm(xseq,b0[1],b1[1]),col=1,lwd=2)
lines(xseq,pnorm(xseq,b0[2],b1[2]),col=2,lwd=2)
lines(xseq,pnorm(xseq,b0[3],b1[3]),col=3,lwd=2)
legend("topleft",legend=c("Full model","Ambient","CO2"),col=1:3,lwd=2)
```

1. Yes, elevated CO2 concentrations have a significant impact on the fecundity of trees. This is demonstrated by the likelihood ratio test; since the p-value is less than 0.5, we can say that the CO2 treatment has a statistically significant impact on tree fecundity. The fecundity plot indicates that elevated CO2 concentrations result in higher fecundity.

2. According to the parameter values and the fecundity and maturation plots, elevated CO2 concentrations produce *both* higher fecundity (greater a0) and maturation rates (maturation occurs at lower mean diameter) than ambient air conditions. 