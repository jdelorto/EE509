...
print(log(1/5))
```
R represents a non-numeric or undefined answer as "NaN".

2. R's trigonometric functions return values in radians, so we must convert them to degrees by multiplying by 180/pi.

3. atan2 allows the user to input the side lengths that define the angle as two separate arguments, rather than the fraction of the two as a single argument.

4. log returns natural log/ln; log10 returns the log in base 10 (more commonly referred to as "log").
...
```
8. Sum returns a single integer value that is the sum of all values in the vector. Cumsum returns a vector that gives the cumulative sum after each position in the original vector.
...
10.
```{r q10}
sequence <- print(seq(-4.8,3.43,length.out=8))
```
a.
```{r q10a}
print((3.43+4.8)/(8-1))
...
12.
```{r q12}
write.table(dat,"my_frogs.txt",row.names=FALSE,sep="|")
```
...
16.
```{r q16}
super <- c("Mr. Incredible","Elastigirl","Frozone","Dynaguy")
```
...
18.
```{r q18}
subset(dat,spots==TRUE)
subset(dat,color=="blue")
sum(subset(dat,color=="blue")[,2])
some_tadpoles <- subset(dat,tadpoles>3 & tadpoles<5)
some_tadpoles
subset(dat,frogs<2.5 & color=="red")
subset(dat,spots==FALSE | frogs>5)
```
...
pairs(dat[c(1,2)])					## all pairwise scatter plots
...
```{r q20}
barplot(tapply(dat$frogs, dat$spots, median),xlab="spots?",ylab="median number of frogs")
```
...
```{r q23}
red_frogs <- subset(dat,color=="red")
red_lm <- lm(tadpoles ~ frogs,data=red_frogs)
summary(red_lm)
plot(red_frogs$frogs,red_frogs$tadpoles)
abline(coef(red_lm)[1],coef(red_lm)[2])

reg_frogs <- lm(tadpoles ~ frogs, data=dat)
summary(reg_frogs)
reg_spots <- lm(tadpoles ~ spots, data=dat)
summary(reg_spots)
reg_color <- lm(tadpoles ~ color, data=dat)
summary(reg_color)
reg_all <- lm(tadpoles ~ frogs*spots*color, data=dat)
summary(reg_all)
```

The only predictor that is significantly correlated with `tadpoles` is `frogs`. However, when all predictors and their interactions are considered, two of the interaction terms (spots/color and frogs/spots/color) are significant as well.
...
```{r q25}
createPlots <- function(data,x.col,y.col,x.lab,y.lab,title){
  reg <- lm(y.col ~ x.col)
  plot(x.col,y.col,
...
many_tadpoles <- subset(dat,tadpoles>=5)
createPlots(many_tadpoles,many_tadpoles$frogs,many_tadpoles$tadpoles,"frogs","tadpoles","frogs v. tadpoles")
```
...
N = rep(N0,NT)
for(t in 2:NT){
	N[t] <- N0*exp(r*(t-1))    ## discrete logistic growth
}
plot(N)
```
